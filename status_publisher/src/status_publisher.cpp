...
StatusPublisher::StatusPublisher(): it_(nh_)
{
  ...
  navStatus_sub_  = nh_.subscribe<actionlib_msgs::GoalStatusArray>("/move_base/status", 5,
                                                                   &StatusPublisher::navStatusCallBack, this);
  navResult_sub_ = nh_.subscribe<move_base_msgs::MoveBaseActionResult>("/move_base/result", 5,
                                                             &StatusPublisher::navResultCallBack,this);
  explStatus_sub_ = nh_.subscribe<actionlib_msgs::GoalStatusArray>("/explore_server/status",5,
                                                                   &StatusPublisher::explStatusCallback, this);
  explResult_sub_ = nh_.subscribe<frontier_exploration::ExploreTaskActionResult>("/explore_server/result", 5,
...
{
  if (!explStatus->status_list.empty()) //First make you the vector is not empty to avoid memory allocation errors.
    {
